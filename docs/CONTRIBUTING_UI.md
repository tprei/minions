...
Gates on **E1** (`continue-task` attachments, merged in alongside UI-8.5).

#### Diff viewer: comment composer

`createDiffViewer` exposes two additional methods: `getComments()` and `onSend(cb)`.

After a multi-line drag selection, a floating "Add comment" button appears at the anchor line. Clicking it shows an inline form with a textarea, **Save**, and **Cancel**. On save:
...
`createComposer` gains three additional methods:
...
The pill label is clickable — it toggles a `.composer-attachment-list` showing one item per attachment. Tapping an item triggers `onCommentJump` listeners.
